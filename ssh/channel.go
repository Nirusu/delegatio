/* SPDX-License-Identifier: AGPL-3.0-only
 * Copyright (c) Benedict Schlueter
 */

package main

import (
	"context"
	"encoding/binary"
	"fmt"
	"sync"

	"github.com/benschlueter/delegatio/cli/kubernetes"
	"go.uber.org/zap"
	"golang.org/x/crypto/ssh"
	"k8s.io/client-go/tools/remotecommand"
)

// SSHChannelServer is a wrapper around an ssh.Channel and ssh.Requests.
type SSHChannelServer struct {
	client    *kubernetes.Client
	channel   ssh.Channel
	requests  <-chan *ssh.Request
	ptyReq    *PtyRequestPayload
	wg        *sync.WaitGroup
	log       *zap.Logger
	done      chan struct{}
	window    *Winsize
	namespace string
	userID    string
}

// NewSSHChannelServer returns a new SSHChannelServer.
func NewSSHChannelServer(channel ssh.Channel, client *kubernetes.Client, log *zap.Logger, requests <-chan *ssh.Request, namespace, userID string) *SSHChannelServer {
	return &SSHChannelServer{
		client:    client,
		log:       log,
		channel:   channel,
		requests:  requests,
		wg:        &sync.WaitGroup{},
		done:      make(chan struct{}),
		window:    &Winsize{Queue: make(chan *remotecommand.TerminalSize)},
		namespace: namespace,
		userID:    userID,
	}
}

// Serve starts the server. It will block until the context is canceled.
func (s *SSHChannelServer) Serve(ctx context.Context) {
	defer s.wg.Wait()
	for {
		select {
		case <-ctx.Done():
			return
		case req, ok := <-s.requests:
			if !ok {
				s.log.Debug("request channel closed")
				return
			}
			s.log.Debug("received data over request channel", zap.Any("req", req))
			switch req.Type {
			case "shell":
				s.wg.Add(1)
				go s.handleShell(ctx)
				if err := req.Reply(true, nil); err != nil {
					s.log.Error("failled to reply to \"shell\" request", zap.Error(err))
				}
			case "pty-req":
				ptyReq := PtyRequestPayload{}
				if err := ssh.Unmarshal(req.Payload, &ptyReq); err != nil {
					s.log.Error("failled to unmarshal pty request", zap.Error(err))
					continue
				}
				s.log.Info("pty request", zap.Any("ptyReq", ptyReq))
				s.ptyReq = &ptyReq
				if err := req.Reply(true, nil); err != nil {
					s.log.Error("failled to respond to \"pty-req\" request", zap.Error(err))
				}
			case "window-change":
				s.window.Queue <- parseDims(req.Payload)
			default:
				if req.WantReply {
					if err := req.Reply(false, nil); err != nil {
						s.log.Error("failled to respond to request", zap.Any("request", req), zap.Error(err))
					}
				}
				s.log.Info("unimplemented request", zap.Any("request", req))
			}
		}
	}
}

func (s *SSHChannelServer) handleShell(ctx context.Context) {
	defer s.wg.Done()
	defer func() {
		if err := s.channel.Close(); err != nil {
			s.log.Error("failed to close channel", zap.Error(err))
		}
	}()
	// Fire up "kubectl exec" for this session
	tty := false
	if s.ptyReq != nil {
		tty = true
	}
	err := s.client.CreatePodShell(ctx, s.namespace, fmt.Sprintf("%s-statefulset-0", s.userID), s.channel, s.window, tty)
	if err != nil {
		s.log.Error("createPodShell exited with errorcode", zap.Error(err))
		_, _ = s.channel.Write([]byte(fmt.Sprintf("closing connection, reason: %v", err)))
		return
	}
	_, _ = s.channel.Write([]byte("graceful termination"))
}

// parseDims extracts terminal dimensions (width x height) from the provided buffer.
func parseDims(b []byte) *remotecommand.TerminalSize {
	w := binary.BigEndian.Uint32(b)
	h := binary.BigEndian.Uint32(b[4:])
	return &remotecommand.TerminalSize{
		Width:  uint16(w),
		Height: uint16(h),
	}
}

// Winsize stores the Height and Width of a terminal.
type Winsize struct {
	Queue chan *remotecommand.TerminalSize
}

// Next sets the size.
func (w *Winsize) Next() *remotecommand.TerminalSize {
	return <-w.Queue
}
